...
class Station:
    def __init__(self, number, name, line):
        ...
        self.nearstations = []  # Соседи: [(номер_соседа, расстояние)]
...
def create_stations_db(file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for station in data['stations']:
            ...
            nearstations_num = station['neighbor_num']
            stations[station_num].number = station_num
            stations[station_num].name = station['name']
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("line_"):
        ...
        if start is not None and finish is not None:
            ...
            result = print_path(STATIONS, dist, intermediate_stations, finish)
